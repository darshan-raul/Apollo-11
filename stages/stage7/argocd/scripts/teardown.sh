#!/bin/bash
# Tear down the Stage 7 ArgoCD GitOps module.
#
...
if [[ "$MODE" == "full" || "$MODE" == "purge" ]]; then
    ...
    kubectl delete -f "$ARGOCD_DIR/../helm/apollo11/bundles/vpa-install.yaml" --ignore-not-found --wait=false >/dev/null 2>&1 || true
    kubectl delete -f "$ARGOCD_DIR/../helm/apollo11/bundles/metrics-server-install.yaml" --ignore-not-found --wait=false >/dev/null 2>&1 || true
    for ns in \
