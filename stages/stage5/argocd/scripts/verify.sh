#!/bin/bash
# Verify the Stage 5 Argo CD module: controller health, project boundaries,
# three isolated Applications, environment workloads, and real self-heal.
set -euo pipefail
...
for crd in applications.argoproj.io gateways.gateway.networking.k8s.io ipaddresspools.metallb.io; do
...
step "3/5 Three isolated Applications"
for env in dev staging prod; do
    ...
    for expected in "namespaces.apps=$apps_ns" "namespaces.ui=$ui_ns" "gateway.createClass=false" "metallb.enabled=false"; do
...
if [[ "$SKIP_WORKLOADS" == "true" ]]; then
    ...
    step "4/5 Dev and staging workloads are independently healthy"
